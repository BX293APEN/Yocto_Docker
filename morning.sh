...
# /boot/grub/grub.cfg  generated by morning.sh (Yocto)
set default=0
set timeout=10
...
CFGEOF
    ...
    if [[ ! -f "${MOUNT_ROOT}/usr/bin/grub-install" ]] && \
       [[ ! -f "${MOUNT_ROOT}/usr/sbin/grub-install" ]]; then
        ...
        warn "  ターゲット PC 上で手動実行が必要です: grub-install --target=x86_64-efi ..."
    else
        log "  chroot 内で grub-install 実行中..."
        TARGET_DEV_FOR_GRUB="${TARGET_DEV}"
        chroot "${MOUNT_ROOT}" /usr/bin/env -i \
            HOME=/root \
            PATH=/usr/bin:/usr/sbin:/bin:/sbin \
            TARGET_DEV="${TARGET_DEV_FOR_GRUB}" \
            /bin/bash << 'GRUB_EOF'
set -e
