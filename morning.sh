...
GRUB_INSTALL_EOF
    ...
    # ── grub.cfg を UUID ベースで生成し全箇所に書き込む ─────────
    # 【重要】Yocto は /boot/EFI/BOOT/grub.cfg を自動生成するが
    # そこには "root=/dev/sda2" がハードコードされており、
    # 別PCで起動するとデバイス名が変わってカーネルパニックになる。
    # morning.sh 側で UUID ベースの正しい grub.cfg を全箇所に上書きする。
    log "  grub.cfg を UUID ベースで生成・上書き中..."

    # /boot/grub/grub.cfg（grub-install が参照）
    mkdir -p "${MOUNT_ROOT}/boot/grub"
    cat > "${MOUNT_ROOT}/boot/grub/grub.cfg" << CFGEOF
# grub.cfg  generated by morning.sh (Yocto) - UUID based
set default=0
set timeout=5
...
menuentry "Yocto Linux" {
    set gfxpayload=text
    linux /boot/${KERNEL} root=UUID=${ROOTFS_UUID} rw rootfstype=ext4 rootwait rootdelay=5 nomodeset console=tty0 net.ifnames=0 biosdevname=0
}

menuentry "Yocto Linux (verbose)" {
    set gfxpayload=text
    linux /boot/${KERNEL} root=UUID=${ROOTFS_UUID} rw rootfstype=ext4 rootwait rootdelay=5 nomodeset console=tty0 loglevel=7 ignore_loglevel net.ifnames=0 biosdevname=0
}
CFGEOF
