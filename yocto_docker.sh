...
MIRROREOF
    ...
    # ── NICドライバ カーネルconfigフラグメント ────────────────────────────────
    # .env の NIC_DRIVERS に CONFIG_XXX シンボルをスペース区切りで指定する。
    # Yocto の linux-yocto bbappend + .cfg フラグメント方式でカーネルに組み込む。
    #
    # 仕組み:
    #   1. BUILD_DIR/recipes-kernel/linux/linux-yocto_%.bbappend を生成
    #      → SRC_URI でフラグメントファイルを追加
    #   2. BUILD_DIR/recipes-kernel/linux/linux-yocto/nic-drivers.cfg を生成
    #      → 指定された CONFIG_XXX=y を列挙（変換テーブル不要・直接書き込み）
    #   3. BBLAYERS に BUILD_DIR を追加（既に BBPATH:prepend 済みだが
    #      bbappend は BBLAYERS 内のレイヤーが対象のため layer.conf も生成）
    #
    # .env の NIC_DRIVERS は CONFIG_XXX シンボルをそのまま指定する形式
    if [[ -n "${NIC_DRIVERS}" ]]; then
        local _nic_recipe_dir="${BUILD_DIR}/recipes-kernel/linux"
        local _nic_cfg_dir="${_nic_recipe_dir}/linux-yocto"
        sudo mkdir -p "${_nic_cfg_dir}"
        sudo chmod -R 777 "${BUILD_DIR}/recipes-kernel"

        # ── nic-drivers.cfg を生成 ───────────────────────────────────────
        # NIC_DRIVERS に指定された CONFIG_XXX シンボルをそのまま =y で書き出す。
        # 先頭が "CONFIG_" で始まらないエントリは誤記として警告しスキップする。
        {
            echo "# NIC driver kernel config fragment"
            echo "# generated by yocto_docker.sh from NIC_DRIVERS=\"${NIC_DRIVERS}\""
            echo ""
            for _cfg in ${NIC_DRIVERS}; do
                if [[ "${_cfg}" == CONFIG_* ]]; then
                    echo "${_cfg}=y"
                    log "NIC ドライバ追加: ${_cfg}=y"
                else
                    warn "NIC_DRIVERS: \"${_cfg}\" は CONFIG_ で始まっていません。スキップします (例: CONFIG_R8169)"
                fi
            done
        } | sudo tee "${_nic_cfg_dir}/nic-drivers.cfg" > /dev/null

        # ── linux-yocto_%.bbappend を生成 ────────────────────────────────
        sudo tee "${_nic_recipe_dir}/linux-yocto_%.bbappend" > /dev/null << 'BBAPPENDEOF'
# NIC driver config fragment (generated by yocto_docker.sh)
FILESEXTRAPATHS:prepend := "${THISDIR}/${PN}:"
SRC_URI:append = " file://nic-drivers.cfg"
BBAPPENDEOF

        # ── BUILD_DIR をレイヤーとして認識させる layer.conf ─────────────
        # BBPATH:prepend は bbclass/local.conf には効くが bbappend の探索には
        # BBLAYERS 登録が必要。BUILD_DIR を動的レイヤーとして登録する。
        local _layer_conf="${BUILD_DIR}/conf/layer.conf"
        if [[ ! -f "${_layer_conf}" ]]; then
            sudo tee "${_layer_conf}" > /dev/null << LAYEREOF
# dynamic layer for kernel config fragments (generated by yocto_docker.sh)
BBPATH .= ":\${LAYERDIR}"
BBFILES += "\${LAYERDIR}/recipes-kernel/linux/*.bbappend"
BBFILE_COLLECTIONS += "yocto-docker-dynamic"
BBFILE_PATTERN_yocto-docker-dynamic = "^\${LAYERDIR}/"
BBFILE_PRIORITY_yocto-docker-dynamic = "99"
LAYERDEPENDS_yocto-docker-dynamic = ""
LAYERVERSION_yocto-docker-dynamic = "1"
LAYERERIES_COMPAT_yocto-docker-dynamic = "${YOCTO_RELEASE}"
LAYEREOF
            sudo chmod 644 "${_layer_conf}"
        fi
        ...
        log "NIC_DRIVERS 未指定 → カーネルconfigフラグメントをスキップします"
    fi
