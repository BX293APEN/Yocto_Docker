...
MIRROREOF
    ...
    # .env の NIC_DRIVERS をスペース区切りで受け取り、
    ...
    if [[ -n "${NIC_DRIVERS}" ]]; then
        ...
        # ── nic-drivers.cfg を生成 ───────────────────────────────────────
        {
            ...
            echo ""
            for _drv in ${NIC_DRIVERS}; do
                local _drv_lower
                _drv_lower=$(echo "${_drv}" | tr '[:upper:]' '[:lower:]')
                local _cfg="${_NIC_MAP[${_drv_lower}]:-}"
                if [[ -n "${_cfg}" ]]; then
                    echo "${_cfg}=y"
                    log "NIC ドライバ追加: ${_drv} → ${_cfg}=y"
                else
                    warn "NIC_DRIVERS: 未知のドライバ名 \"${_drv}\" をスキップします"
                fi
            done
...
LAYEREOF
        ...
        if ! grep -q "${BUILD_DIR}" "${BUILD_DIR}/conf/bblayers.conf" 2>/dev/null; then
            sed -i "s|^\(BBLAYERS ?= \"\)|  ${BUILD_DIR} \\\\\n\1|" \
                "${BUILD_DIR}/conf/bblayers.conf" || true
            # sed が効かない場合は末尾の " の前に追記
            if ! grep -q "${BUILD_DIR}" "${BUILD_DIR}/conf/bblayers.conf" 2>/dev/null; then
                sed -i "s|^\"\$|  ${BUILD_DIR}\n\"|" \
                    "${BUILD_DIR}/conf/bblayers.conf" || true
            fi
